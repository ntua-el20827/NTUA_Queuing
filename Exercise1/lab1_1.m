...
for i = 1 : columns(lambda)
  poisson(i, :) = poisspdf(k, lambda(i)); # Ορίζω την Poisson
endfor
...
hold on;
for i = 1 : columns(lambda)
  stem(k, poisson(i, :), colors(i), "linewidth", 1.2); # Για να φτιαξω τις γρφικες χρησιμοποιω την stem
endfor
hold off;
...
chosen = poisson(index, :); #Διαλεγω απο αυτες που εχω φτιαξει την Poisson με λ=30
mean_value = 0;
for i=0:(columns(poisson(index, :)) - 1) #Υπολογισμος Μεσης Τιμης
